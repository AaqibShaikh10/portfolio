...
export const blogPosts: BlogPost[] = [
    {
        id: 1,
        slug: 'how-i-built-a-real-time-ai-pipeline-for-healthcare-data',
        title: 'How I Built a Real-Time AI Pipeline for Healthcare Data',
        date: '2025-03-10',
        readTime: '9 min read',
        excerpt:
            'A deep dive into the architecture behind a clinical prediction system: streaming ingestion, feature engineering under latency constraints, and why most ML pipelines fail at the inference layer.',
...
`,
    },
    {
        ...
        readTime: '8 min read',
        excerpt:
            'Five hard-won lessons from taking AI systems from prototype to production: why eval harnesses matter more than model selection, and how to think about hallucination in high-stakes contexts.',
        tags: ['AI', 'MLOps', 'Production Systems', 'Software Engineering'],
        content: `
<p>Most AI tutorials stop at the demo. The demo works perfectly: clean data, a cooperative model, a happy path that produces impressive output. Production is different. Production has malformed inputs, users who do unexpected things, infrastructure that occasionally hiccups, and no one watching the system at 3am when it starts behaving oddly.</p>
...
<p>For a retrieval-augmented generation system, for example, the model might handle questions about your product documentation perfectly, then confidently hallucinate when asked about a niche edge case that wasn't in the retrieved context. The solution isn't a better model; it's better retrieval and explicit uncertainty signaling.</p>
...
<p>Workers run at Cloudflare's edge, across over 300 data centers worldwide. Every request is handled by a Worker instance that's already warm (Cloudflare uses a V8 isolate model rather than containers, which eliminates cold starts at the cost of some Node.js API compatibility). The execution environment is a subset of the Web Platform APIs: <code>fetch</code>, <code>Request</code>, <code>Response</code>, <code>crypto</code>, and so on. Familiar to frontend developers.</p>
...
<p>The <code>RESEND_API_KEY</code> is stored as a Cloudflare Pages environment variable, not in source. The function receives it via <code>context.env</code> with a typed interface, which is a clean pattern.</p>
...
<p>The static data (GitHub profile, project list) is fetched at build time and baked into the bundle as a JSON file. No runtime API calls means no latency from that source. This makes the stats page fast and resilient even if GitHub's API is slow or rate-limited.</p>
